        ...
        <div class="container text-white">
            <c:choose>
                <c:when test="${form.equals('alterar')}">
                    <h1 class="text-center">Editar ${roupa.nome}</h1>
                </c:when>
                <c:otherwise>
            ...
            <form action="<c:choose>
                ...
                <c:if test="${form.equals('alterar')}">
                    <div class="form-group d-none">
                        ...
                        <input class="form-control" type="number" name="roupa-id" value="${roupa.id}"></input>
                    </div>
                </c:if>
                <div class="form-group mt-5 <c:if test="${form.equals('alterar')}">d-none</c:if>">
                    <labe for="roupa-nome">Nome</labe>
                    <input class="form-control" type="text" name="roupa-nome" <c:if test="${form.equals('alterar')}">value="${roupa.nome}"</c:if> required></input>
                </div>
                <div class="form-group mt-2 <c:if test="${form.equals('alterar')}">mt-5</c:if>">
                    <labe for="roupa-prazo">Prazo</labe>
                    <input class="form-control" type="number" name="roupa-prazo" <c:if test="${form.equals('alterar')}">value="${roupa.prazoLavagem}"</c:if> required></input>
                </div>
                <div class="form-group mt-2">
                    <labe for="roupa-preco">Preco</labe>
                    <input class="form-control" type="text" name="roupa-preco" <c:if test="${form.equals('alterar')}">value="${roupa.custoLavagem}"</c:if> required></input>
                </div>
                <button class="btn btn-success mt-2" type="submit">Salvar</button>
                <a class="btn btn-danger mt-2" href="RoupasServlet">Cancelar</a>
            </form>
        </div>
    </body>
</html>
